...
//        String target = "/home/huyuling/桌面/Lab/src/test1.txt";
    ...
    public static class MyVisitor extends SysYParserBaseVisitor<LLVMValueRef>{
        ...
        final LLVMValueRef two = LLVMConstInt(i32Type, 2, /* signExtend */ 0);
        ...
        public LLVMValueRef visitFuncDef(SysYParser.FuncDefContext ctx) {
            ...
            int aIndex = 0;
            if(size>0) {
                for (SysYParser.FuncFParamContext funcFParam : ctx.funcFParams().funcFParam()) {
                    if (funcFParam.L_BRACKT().size() == 0) {
                        argumentTypes.put(aIndex++, i32Type);
                    } else {
                        argumentTypes.put(aIndex++, LLVMPointerType(i32Type, 0));
                    }
                }
            }
            ...
            if(ctx.funcFParams()!=null){
                ...
                for(SysYParser.FuncFParamContext paramContext:ctx.funcFParams().funcFParam()){
                    ...
                    if (paramContext.L_BRACKT().size() == 0) {
                        ...
                        baseSymbol = new BaseSymbol(s, pointer);
                    }else{
                        LLVMValueRef pointer = LLVMBuildAlloca(builder, LLVMPointerType(i32Type,0), /*pointerName:String*/s);
                        LLVMBuildStore(builder, n, pointer);
                        baseSymbol = new BaseSymbol(s, pointer);
                        baseSymbol.setArray(true);
                    }
                    baseSymbol.setFParam(true);
...
//                LLVMBuildLoad(builder, left, ctx.lVal().IDENT().getText());
                ...
                if(ctx.lVal().exp(0) instanceof SysYParser.NumContext){
                    int index = convert(((SysYParser.NumContext) ctx.lVal().exp(0)).number().INTEGR_CONST().getText());
                    valuePointer= new PointerPointer(new LLVMValueRef[]{zero, LLVMConstInt(i32Type, index, 0)});
                }else{
                    LLVMValueRef value = visit(ctx.lVal().exp(0));
                    if(symbol.isFParam){
                        valuePointer = new PointerPointer(new LLVMValueRef[]{value});
                    }else {
                        valuePointer = new PointerPointer(new LLVMValueRef[]{zero, value});
                    }
                }
                // NullPointerException
                if(symbol.isFParam){
                    LLVMValueRef res = LLVMBuildLoad(builder, symbol.getLlvmValueRef(), "res");
                    left = LLVMBuildGEP(builder, res, valuePointer,1,"res");
                }else {
...
//                LLVMBuildLoad(builder, left, ctx.lVal().IDENT().getText());
            ...
            if (ctx.exp() instanceof SysYParser.NumContext) {
                ...
                if(symbol.isFParam){
                    newSymbol.setFParam(true);
                }
                if(symbol.isArray){
                    newSymbol.setArray(true);
                }
        ...
        public LLVMValueRef visitConstDef(SysYParser.ConstDefContext ctx) {
            ...
            if(ctx.constExp().size() == 0){
                ...
                LLVMTypeRef vectorType = LLVMArrayType(i32Type, leftSize);
                ...
                if(currentScope == globalScope){
                    LLVMValueRef valueRef = LLVMConstArray(vectorType, arguments, leftSize);
        ...
        public LLVMValueRef visitStmt5(SysYParser.Stmt5Context ctx) {
            LLVMBasicBlockRef whileConditionBlock = LLVMAppendBasicBlock(this.function, "whileCondition");
            ...
            LLVMValueRef tmp_ = visit(ctx.cond());
            tmp_ = LLVMBuildICmp(builder, LLVMIntNE, LLVMConstInt(i32Type, 0, 0), tmp_, "tmp_");
            LLVMBasicBlockRef whileBodyBlock = LLVMAppendBasicBlock(this.function, "whileBody");
            LLVMBasicBlockRef entryBlock = LLVMAppendBasicBlock(this.function, "entry");
            whileBlocks.push(entryBlock);
            LLVMBuildCondBr(builder,
                    /*condition:LLVMValueRef*/ tmp_,
                    /*ifTrue:LLVMBasicBlockRef*/ whileBodyBlock,
                    /*ifFalse:LLVMBasicBlockRef*/ entryBlock);
         ...
         */
        @Override
        public LLVMValueRef visitStmt6(SysYParser.Stmt6Context ctx) {
            LLVMBasicBlockRef obj = whileBlocks.peek();
            LLVMBuildBr(builder, obj);
            return null;
        }

        @Override
        public LLVMValueRef visitStmt7(SysYParser.Stmt7Context ctx) {
            LLVMBasicBlockRef obj = whileConditionBlocks.peek();
            LLVMBuildBr(builder, obj);
            return null;
        }
        ...
        public LLVMValueRef visitCond(SysYParser.CondContext ctx) {
            ...
            if(ctx.NEQ()!=null){ // !=
                ...
                tmp_ = LLVMBuildICmp(builder, LLVMIntSLT, left, right, "tmp_");
            }else if(ctx.GT()!=null){
                tmp_ = LLVMBuildICmp(builder, LLVMIntSGT, left, right, "tmp_");
            }else if(ctx.LE()!=null){
                tmp_ = LLVMBuildICmp(builder, LLVMIntSLE, left, right, "tmp_");
            }else if(ctx.GE()!=null){
                tmp_ = LLVMBuildICmp(builder, LLVMIntSGE, left, right, "tmp_");
...
//                tmp_ = LLVMBuildICmp(builder, LLVMOr, left, right, "tmp_");
        ...
        public LLVMValueRef visitLVal(SysYParser.LValContext ctx) {
            ...
            if(ctx.exp().size() == 0){
                if(symbol instanceof BaseSymbol && ((BaseSymbol) symbol).isArray){
                    LLVMValueRef pointer = symbol.getLlvmValueRef();
                    if(((BaseSymbol) symbol).isFParam){
                        return LLVMBuildLoad(builder, pointer, "res");
                    }
                    PointerPointer valuePointer = new PointerPointer(new LLVMValueRef[]{zero, zero});
                    LLVMValueRef res = LLVMBuildGEP(builder, pointer, valuePointer, 2, "res");
                    return res;
                }else {
                    LLVMValueRef pointer = symbol.getLlvmValueRef();
                ...
                if(symbol instanceof BaseSymbol){
                    if(!((BaseSymbol) symbol).getFParam()) {
                        ...
                        LLVMValueRef pointer = LLVMBuildLoad(builder, vectorPointer, name);
                        PointerPointer valuePointer = new PointerPointer(new LLVMValueRef[]{value});
                        LLVMValueRef res = LLVMBuildGEP(builder, pointer, valuePointer,1,"res");
                        return LLVMBuildLoad(builder, res, name);
                    }
                }
                return null;
